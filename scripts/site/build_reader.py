...
DEFINITIONS = {
    ...
    "PottsCI.PinningData.DegreeBound": (r"\deg_{G^\tau}(u)+\sum_c b^\tau_u(c)\le\Delta",
        "Each free vertex has at most Δ free and pinned neighbours in total."),
    ...
    "PottsCI.PinningData.hardParameter": ("x=0",
        "The activity x = 0, where only proper colourings carry weight."),
    ...
    "ZeroFreeness.Appendix.CV.betaPlus": (r"\beta^{+}_x",
        "β⁺_x = q − P₂θL_x + (1 + P₂)Δ + 2, the coefficient of the expected discount loss."),
    ...
    "ZeroFreeness.Appendix.CV.bulk": (r"\Lambda_{\rm bulk}",
        "Λ_bulk = 4/3 + P₂ + (4/3)P₄ = 1331/750, the rate for high multiplicity."),
    "ZeroFreeness.Appendix.CV.canonicalColourCharge": ("",
        "The record upper charge of one off-root colour c (the paper’s cv-charge)."),
    "ZeroFreeness.Appendix.CV.correctedHardColourCharge": ("",
        "The charge of one colour for one active outcome, corrected by γ_gain and γ_loss."),
    ...
    "ZeroFreeness.Appendix.CV.cvKernel": (r"K_{x,\mathrm{CV}}^{G,\tau}",
        "The soft Carlson–Vigoda flip kernel at activity x."),
    ...
    "ZeroFreeness.Appendix.CV.gammaGain": (r"\gamma_{\rm gain}",
        "γ_gain = η(ϱ)(ϱ − 1 − 2/Δ), the per-Δ metric gain."),
    "ZeroFreeness.Appendix.CV.gammaLoss": (r"\gamma_{\rm loss}",
        "γ_loss = η(ϱ)(ϱ − P₂θφ_x + 1 + P₂ + 2/Δ), the per-Δ metric loss."),
    "ZeroFreeness.Appendix.CV.gammaSet": (r"\Gamma_c",
        "Neighbours u of v with X_u = Y_u = c whose root edge is active on both sides."),
    "ZeroFreeness.Appendix.CV.gap": ("",
        "The contraction margin 59/226125 left by the final inequality."),
    ...
    "ZeroFreeness.Appendix.CV.incidenceDiscount": ("s_u",
        "The discount s_u = θx^{r_u+e_u} of the root edge to u in the metric d_x."),
    "ZeroFreeness.Appendix.CV.low": (r"\Lambda_{\rm low}",
        "Λ_low = max{Λ₁, Λ₂, Λ₃}, the rate charged per low-multiplicity incidence."),
    "ZeroFreeness.Appendix.CV.lowAvailabilityMass": ("L_x",
        "The x-weighted count of root incidences of low-multiplicity colours."),
    ...
    "ZeroFreeness.Appendix.CV.rootColourCharge": ("",
        "The charge of the two root colours a and b."),
    ...
    "ZeroFreeness.Appendix.Girth.largeGirthFamily": (r"\operatorname{girth}(G^\tau)\ge g",
        "The residual instances whose free graph G^τ has girth at least g."),
    ...
    "ZeroFreeness.Potts.ExternalCriticalHardColouringTheorem": (r"\mathcal G_{\Delta}\text{ at }x=0",
        "𝒢_Δ satisfies coupling independence at x = 0 with some finite constant."),
    ...
    "ZeroFreeness.Potts.FlipProfile.R": (r"R_\rho",
        "R_ρ = sup_{s≥1} s ρ_s."),
    "ZeroFreeness.Potts.HasSmallResponseLog": (r"\bigl|\operatorname{Log}\tfrac{f(z)/f(x)}{g(z)/g(x)}\bigr|\le\alpha",
        "A holomorphic logarithm of the response ratio on |z − x| < ε, zero at x, of modulus ≤ α."),
    ...
    "ZeroFreeness.Potts.PositiveIntervalCI": (r"C\text{-coupling independence on }[\delta,1]",
        "The class satisfies C-coupling independence at every activity x ∈ [δ, 1]."),
    ...
    "ZeroFreeness.Potts.actualRootChildGibbs": (r"\mu_{G,x}^{\tau^{r=a}}",
        "The Gibbs law at x ≥ 0 of the child τ^{r=a}, on V^τ ∖ {r}."),
    "ZeroFreeness.Potts.addBoundary": (r"b_r(a)\mapsto b_r(a)+1",
        "The system with one more free–pinned edge, from r to a pinned vertex coloured a."),
    ...
    "ZeroFreeness.Potts.pinVertex": (r"\tau^{v=a}",
        "The pinning τ extended by pinning the free vertex v to colour a."),
    "ZeroFreeness.Potts.positiveRootChildGibbs": (r"\mu_{G,x}^{\tau^{r=a}}",
        "The Gibbs law at x > 0 of the child τ^{r=a}, on V^τ ∖ {r}."),
    ...
    "ZeroFreeness.Potts.softFlipKernel": (r"K_{x,\rho}^{G,\tau}",
        "The soft flip kernel: sample active constraints, apply the hard flip of profile ρ, forget them."),
