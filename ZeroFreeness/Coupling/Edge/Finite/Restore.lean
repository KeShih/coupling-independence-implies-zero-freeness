import ZeroFreeness.Coupling.Edge.Finite.Odds
...
theorem W_restore_le (e : E) (a b : A)
    ...
    W ham (ZeroFreeness.mapLaw μ (joinConfig e a)) (ZeroFreeness.mapLaw ν (joinConfig e b)) ≤ 1 + W ham μ ν := by
  obtain ⟨π, hπ⟩ := exists_optimal_coupling μ ν ham ham_nonneg
  apply (W_le_cost ham_nonneg (ZeroFreeness.mapCoupling π (joinConfig e a) (joinConfig e b))).trans
  rw [ZeroFreeness.cost_mapCoupling]
  calc
    _ ≤ π.cost (fun σ τ => 1 + ham σ τ) := ZeroFreeness.cost_le_cost π (ham_join_le e a b)
    _ = 1 + π.cost ham := by
