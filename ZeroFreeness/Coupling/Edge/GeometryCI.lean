import ZeroFreeness.Coupling.Edge.PottsCI
...
open PottsCI PottsCI.FinDist ZeroFreeness.Potts EndpointGeometry FiniteSystem Filter
...
theorem root_children_ci (g : EndpointGeometry V E) (τ : PartialColouring E C)
  ...
  · have hx : x = PinningData.hardParameter := Subtype.ext hz
    ...
    let μ (c : C) (n : ℕ) := (rootChildData τ g.edgeGraph r c).gibbs (ZeroFreeness.hardApproach n)
      (ZeroFreeness.hardApproach n).property
      ((rootChildData τ g.edgeGraph r c).partition_pos_of_parameter_pos (ZeroFreeness.hardApproach_pos n))
    ...
    exact Filter.Eventually.of_forall fun n => g.root_children_ci_positive τ r a b hΔ hdegree hq
      (ZeroFreeness.hardApproach n) (ZeroFreeness.hardApproach_pos n) (ZeroFreeness.hardApproach_le_one n)
