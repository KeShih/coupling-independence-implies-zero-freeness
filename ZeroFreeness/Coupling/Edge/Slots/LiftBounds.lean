import ZeroFreeness.Coupling.Edge.Slots.LiftLimit
import ZeroFreeness.Coupling.Edge.Slots.Definitions
...
lemma finiteSlotCoordinate_sum (κ : R → ℝ) (hκ : ∀ r, 0 ≤ κ r)
  ...
  have hm := ZeroFreeness.expectReal_product_coordinate (fun _ : Fin 2 => p) h
    (fun a => if a = r then (1 : ℝ) else 0)
  simpa [expectReal, ZeroFreeness.productLaw, p, mul_ite] using hm
