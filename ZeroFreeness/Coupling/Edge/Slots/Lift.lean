...
import ZeroFreeness.Coupling.Edge.Slots.LiftFactorization
...
lemma slotSystem_gibbs_colour_weight (g : EndpointGeometry V E) (κ : R → ℝ)
    ...
    (ZeroFreeness.mapLaw ((g.slotSystem κ hκ x hx b).gibbs (fun _ => ∅) hZ) colourProjection).w φ =
      g.colourFibreWeight κ x b φ / (g.slotSystem κ hκ x hx b).partition (fun _ => ∅) := by
  unfold ZeroFreeness.mapLaw FinDist.bind FinDist.pure
