import ZeroFreeness.Coupling.CLMM.Ambient
...
namespace ZeroFreeness.Appendix.CLMM.Lemma513
open scoped BigOperators
open PottsCI PottsCI.FinDist ZeroFreeness ZeroFreeness.Potts ZeroFreeness.Appendix.Edge.FiniteLaw
...
theorem recursion_case {Δ g R : ℕ} {x ε : ℝ} (hΔ : 3 ≤ Δ) (hR : 2 ≤ R) (hx : 0 < x)
    ...
    (hS : ZeroFreeness.Appendix.CLMM.FixedAmbientSphereDecay.{u,v} C Δ g R x ε)
  ...
  let TV : W → ℝ := fun w => (1 / 2 : ℝ) * ∑ c,
    |ZeroFreeness.Appendix.CLMM.singleSiteMass (DoobPinning.law (optionChildData (restrictPinningData J f p) a) x hx) w c -
      ZeroFreeness.Appendix.CLMM.singleSiteMass (DoobPinning.law (optionChildData (restrictPinningData J f p) b) x hx) w c|
  obtain ⟨w₀, hw₀S, hmin⟩ := S.exists_min_image TV hSne
  have hw₀ : J.graph.edist (f none) (f (some w₀)) = R := (Finset.mem_filter.mp hw₀S).2
  have hinf : ZeroFreeness.Appendix.CLMM.ambientSphereInfluence J.graph f R
  ...
  have hsum : ZeroFreeness.Appendix.CLMM.ambientSphereInfluence J.graph f R
      (DoobPinning.law (optionChildData (restrictPinningData J f p) a) x hx)
      (DoobPinning.law (optionChildData (restrictPinningData J f p) b) x hx) = ∑ w ∈ S, TV w := by
    unfold ZeroFreeness.Appendix.CLMM.ambientSphereInfluence
...
end ZeroFreeness.Appendix.CLMM.Lemma513
