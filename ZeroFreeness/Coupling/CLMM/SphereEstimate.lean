...
`ZeroFreeness.Appendix.CLMM.Literature` from tree total-influence decay and
ratio-form relative SSM (Lemmas 5.19 and 5.20 of CLMM2023).
-/

namespace ZeroFreeness.Appendix.CLMM.Eq10
open scoped BigOperators
open Finset ZeroFreeness.Appendix.Girth
...
theorem tree_sameDomain (y' : X → C) : ∀ (n : ℕ) (v : X),
    ZeroFreeness.Appendix.CLMM.SameDomain (tree H d b y n v) (tree H d b y' n v) := by
  intro n
  induction n with
  | zero =>
    intro v
    refine ZeroFreeness.Appendix.CLMM.SameDomain.node 0 _ _ Fin.elim0 Fin.elim0 ?_ (fun i => Fin.elim0 i)
    rw [Finset.sum_add_distrib, Finset.sum_add_distrib, sum_filter_card, sum_filter_card]
  | succ n ih =>
    intro v
    exact ZeroFreeness.Appendix.CLMM.SameDomain.node _ (b v) (b v) _ _ rfl (fun i => ih _)
...
theorem local_tv (Δ : ℕ) {x : ℝ} (hx : 0 < x) (A ρ : ℝ)
    (hTID : ZeroFreeness.Appendix.CLMM.TreeTID (C := C) Δ x hx A ρ) (k r : ℕ) (hL : L = k + 1)
...
theorem tv_bound (Δ : ℕ) {x : ℝ} (hx : 0 < x) (B ρ : ℝ) (K₀ : ℕ)
    (hRel : ZeroFreeness.Appendix.CLMM.TreeRelative (C := C) Δ x B ρ K₀) (k : ℕ) (hL : L = k + 1)
...
theorem main_bound (Δ : ℕ) {x : ℝ} (hx : 0 < x) (A B ρ : ℝ) (hA : 0 < A) (hB : 0 < B)
    (hρ : 0 < ρ) (K₀ : ℕ)
    (hTID : ZeroFreeness.Appendix.CLMM.TreeTID (C := C) Δ x hx A ρ)
    (hRel : ZeroFreeness.Appendix.CLMM.TreeRelative (C := C) Δ x B ρ K₀)
...
section Bridge
open PottsCI ZeroFreeness.Potts ZeroFreeness.Appendix.CLMM
...
theorem sphere_estimate_proof :
  ...
  have hdegI : ∀ v, I.graph.degree v + ∑ c, I.boundaryCount v c ≤ Δ :=
    ZeroFreeness.Appendix.CLMM.restrict_degreeBound J e pin hpin hJd
...
end ZeroFreeness.Appendix.CLMM.Eq10
