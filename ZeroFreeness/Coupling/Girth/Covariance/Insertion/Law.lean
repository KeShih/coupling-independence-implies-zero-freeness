import ZeroFreeness.Coupling.Girth.Spectral.Hoeffding
...
def conditionalLaw (ξ : Ω) : FinDist ((U → C) × O) :=
  kernelJoint (ZeroFreeness.productLaw (M.cavity ξ)) (fun _ => M.exterior ξ)
...
theorem expect_F (s : ℝ) (c : C) (ξ : Ω) :
    expectReal (ZeroFreeness.productLaw (M.cavity ξ)) (M.F s c) = M.Y s c ξ := by
  change expectReal (ZeroFreeness.productLaw (M.cavity ξ))
    (fun σ => ∏ u, (1 - s * colourIndicator c (σ u))) = ∏ u, (1 - s * M.pi u c ξ)
  rw [ZeroFreeness.expectReal_productLaw (M.cavity ξ) (fun _ t => 1 - s * colourIndicator c t)]
...
theorem expect_residual (s : ℝ) (c : C) (ξ : Ω) :
    expectReal (ZeroFreeness.productLaw (M.cavity ξ)) (M.residual s c) = M.G s c ξ := by
  change expectReal (ZeroFreeness.productLaw (M.cavity ξ))
  ...
  rw [M.expect_F]
  simp_rw [ZeroFreeness.expectReal_product_coordinate, expect_colourIndicator]
  rfl
