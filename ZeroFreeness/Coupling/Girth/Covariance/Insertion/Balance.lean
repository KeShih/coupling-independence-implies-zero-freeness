...
import ZeroFreeness.Coupling.Girth.Spectral.Graph.PositivePoincare
...
theorem product_swap (p : U → FinDist C) (i : U) (α : U → C) (c : C) :
    (ZeroFreeness.productLaw p).w (Function.update α i c) * (p i).w (α i) =
      (ZeroFreeness.productLaw p).w α * (p i).w c := by
...
theorem weight_factorization (hπ : ∀ ξ u c, 0 < (π ξ u).w c)
    ...
    μ.w (ξ,(α,o)) = exteriorMass μ ξ o * (ZeroFreeness.productLaw (π ξ)).w α := by
  let ρ := ZeroFreeness.productLaw (π ξ)
...
theorem law_eq_model (hπ : ∀ ξ u c, 0 < (π ξ u).w c)
  ...
  change μ.w (ξ,(α,o)) = (shellLaw μ).w ξ *
    ((ZeroFreeness.productLaw (π ξ)).w α * (exteriorMass μ ξ o / (shellLaw μ).w ξ))
