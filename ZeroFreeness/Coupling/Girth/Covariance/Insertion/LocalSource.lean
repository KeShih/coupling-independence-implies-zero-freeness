...
import ZeroFreeness.Coupling.Girth.Covariance.Insertion.Affine
...
theorem product_coordinate_tensor (p : U → FinDist C) (u : U) (f : C → ℝ) (g : U → C → ℝ) :
    expectReal (ZeroFreeness.productLaw p) (fun σ => f (σ u) * ∏ j, g j (σ j)) =
  ...
  have hp (σ : U → C) : (∏ j, h j (σ j)) = f (σ u) * ∏ j, g j (σ j) := by
    ...
    ring
  calc
    _ = expectReal (ZeroFreeness.productLaw p) (fun σ => ∏ j, h j (σ j)) := by simp_rw [hp]
    _ = ∏ j, expectReal (p j) (h j) := ZeroFreeness.expectReal_productLaw p h
    _ = _ := by
      rw [← Finset.prod_erase_mul _ _ (Finset.mem_univ u)]
...
theorem product_coordinate_covariance (p : U → FinDist C) (u : U) (f : C → ℝ) (g : U → C → ℝ) :
    covariance (ZeroFreeness.productLaw p) (fun σ => f (σ u)) (fun σ => ∏ j, g j (σ j)) =
      covariance (p u) f (g u) * ∏ j ∈ Finset.univ.erase u, expectReal (p j) (g j) := by
  rw [covariance_eq_moment, product_coordinate_tensor, ZeroFreeness.expectReal_product_coordinate,
    ZeroFreeness.expectReal_productLaw p g,
...
theorem product_coordinate_pair_covariance (p : U → FinDist C) (u v : U) (f g : C → ℝ) :
    covariance (ZeroFreeness.productLaw p) (fun σ => f (σ u)) (fun σ => g (σ v)) =
...
theorem conditional_coordinate_residual (s : ℝ) (ξ : Ω) (u : U) (f : C → ℝ) (c : C) :
    covariance (ZeroFreeness.productLaw (M.cavity ξ)) (fun σ => f (σ u)) (M.residual s c) =
      M.beta s u c ξ * covariance (M.cavity ξ u) f (colourIndicator c) := by
  change covariance (ZeroFreeness.productLaw (M.cavity ξ)) (fun σ => f (σ u))
  ...
  change (M.T s c)⁻¹ * covariance (ZeroFreeness.productLaw (M.cavity ξ)) (fun σ => f (σ u))
...
theorem conditional_local_residual (s : ℝ) (ξ : Ω) (f : U → C → ℝ) (c : C) :
    covariance (ZeroFreeness.productLaw (M.cavity ξ)) (fun σ => ∑ u, f u (σ u)) (M.residual s c) =
