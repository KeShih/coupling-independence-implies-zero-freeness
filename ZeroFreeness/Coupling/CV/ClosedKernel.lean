import ZeroFreeness.Coupling.CV.SoftEndgame
...
open PottsCI PottsCI.FinDist ZeroFreeness.Appendix.CV ZeroFreeness.Potts Filter Topology
...
def cvKernel [Nonempty V] [Nonempty C] (I : PinningData V C) (x : ℝ)
  ...
  (activityCoins I x hx).bind fun ω =>
    ZeroFreeness.Appendix.CV.hardStep (PottsCI.Vigoda.activeHardListInstance I (activatedSet I X ω)) X
...
theorem cvKernel_one [Nonempty V] [Nonempty C] (I : PinningData V C) (X : V → C) :
    cvKernel I 1 (mem_Icc_of zero_lt_one le_rfl) X =
      ZeroFreeness.Appendix.CV.hardStep (PottsCI.Vigoda.activeHardListInstance I ∅) X := by
...
theorem cvKernel_stationary [Nonempty V] [Nonempty C] (I : PinningData V C)
  ...
  · have hK : cvKernel I 1 (mem_Icc_of hx0 hx1) =
        ZeroFreeness.Appendix.CV.hardStep (PottsCI.Vigoda.activeHardListInstance I ∅) :=
    ...
    have hlaw : I.gibbs 1 hx0.le (I.partition_pos_of_parameter_pos hx0) =
      ...
      simp
    rw [hK, hlaw]
    exact ZeroFreeness.Appendix.CV.hardStep_stationary _ hne
...
theorem diagMix_cost {Ω S T : Type*} [Fintype Ω] [Fintype S] [Fintype T] (α : FinDist Ω)
    (K : Ω → FinDist S) (L : Ω → FinDist T) (κ : ∀ ω, Coupling (K ω) (L ω))
    (d : S → T → ℝ) :
    (diagMix α K L κ).cost d = ZeroFreeness.expectReal α fun ω => (κ ω).cost d := by
  unfold Coupling.cost diagMix ZeroFreeness.expectReal
...
theorem cv_contraction_of_regime [Nonempty V] [Nonempty C] (I : PinningData V C) {Δ : ℕ}
  ...
  obtain ⟨v, hv, hagree⟩ := ZeroFreeness.exists_unique_disagreement hXY
...
theorem cvKernel_addBoundary_W_le [Nonempty V] [Nonempty C]
  ...
  have hold : (commonCoinLaw (K := I.Constraint) (1 - x) ht).bind
      (fun ω => ZeroFreeness.Appendix.CV.hardStep (PottsCI.Vigoda.activeHardListInstance I (activatedSet I X ω)) X) =
      p.bind (fun ω => ZeroFreeness.Appendix.CV.hardStep
  ...
  calc
    _ ≤ ∑ ω, p.w ω * (if ω (freshBoundaryConstraint I r a) = true then
      ...
      exact mul_le_mul_of_nonneg_left
        (ZeroFreeness.Appendix.CV.activated_hardStep_addBoundary_W_le I r a X ω) (p.nonneg ω)
    _ = (1 - x) / ((Fintype.card V : ℝ) * (Fintype.card C : ℝ)) := by
      ...
      simpa [p, commonCoinLaw, ZeroFreeness.expectReal, bernoulliLaw, div_eq_mul_inv] using h
