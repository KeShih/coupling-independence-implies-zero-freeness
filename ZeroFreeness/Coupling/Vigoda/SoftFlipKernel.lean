...
import ZeroFreeness.Coupling.Vigoda.SoftFlipBoundary
...
`ZeroFreeness.Appendix.CV.hardStep` and `softCVKernel` on their common domain.
-/

namespace ZeroFreeness.Potts

open PottsCI PottsCI.FinDist PottsCI.Vigoda ZeroFreeness ZeroFreeness.Potts
...
def cvProfile : FlipProfile where
  ρ := ZeroFreeness.Appendix.CV.mass
  nonneg s _ := ZeroFreeness.Appendix.CV.mass_nonneg s
  le_one s _ := ZeroFreeness.Appendix.CV.mass_le_one s
...
theorem cvProfile_R : cvProfile.R = 1 := by
  apply le_antisymm
  · exact cvProfile.R_le fun (s : ℕ) _ => ZeroFreeness.Appendix.CV.size_mass_le_one s
  · have h := cvProfile.le_R (cvProfile.RFinite_of_bound
      fun (s : ℕ) _ => ZeroFreeness.Appendix.CV.size_mass_le_one s) (s := 1) le_rfl
    simpa [cvProfile, ZeroFreeness.Appendix.CV.mass] using h

theorem cvProfile_RFinite : cvProfile.RFinite :=
  cvProfile.RFinite_of_bound fun (s : ℕ) _ => ZeroFreeness.Appendix.CV.size_mass_le_one s
...
theorem profileStep_cv [Nonempty V] [Nonempty C] (F : HardListInstance V C)
    (X : V → C) : profileStep cvProfile F X = ZeroFreeness.Appendix.CV.hardStep F X := rfl
...
theorem softFlipKernel_cv [Nonempty V] [Nonempty C] (I : PinningData V C)
    (x : ℝ) (hx0 : 0 < x) (hx1 : x < 1) :
    softFlipKernel cvProfile I x hx0 hx1.le = ZeroFreeness.Appendix.CV.softCVKernel I x hx0 hx1 := by
  funext σ
  apply FinDist.ext
  funext τ
  rw [softFlipKernel_w]
...
theorem softCVKernel_reversible_irreducible [Nonempty V] [Nonempty C]
    (I : PinningData V C) (x : ℝ) (hx0 : 0 < x) (hx1 : x < 1) :
    IsReversible (ZeroFreeness.Appendix.CV.softCVKernel I x hx0 hx1)
        (I.gibbs x hx0.le (I.partition_pos_of_parameter_pos hx0)) ∧
      IsIrreducible (ZeroFreeness.Appendix.CV.softCVKernel I x hx0 hx1) := by
...
theorem rootChild_softCV_boundary_W_le [Nonempty C]
  ...
  have h := rootChild_softFlip_boundary_W_le cvProfile
    (fun (s : ℕ) _ => ZeroFreeness.Appendix.CV.size_mass_le_one s) tau G r a hdegree X x hx0 hx1
  simpa using h
