import ZeroFreeness.Coupling.Vigoda.HardRootColours

/-! Root-colour charges computed from the actual hard transition laws. -/
...
local instance rootColourChargeColouringDecEq : DecidableEq (V → C) := Classical.decEq _
...
lemma scaledMoveMass_of_allowed (F : HardListInstance V C) (X : V → C)
    ...
    scaledMoveMass F X u c = vigodaMass (flipSet F.graph X u c).card := by
...
lemma root_colour_holding_charge_le
    ...
    (ham X (flipConfiguration Y (flipSet FY.graph Y v a) (Y v) a) - 1) *
      scaledMoveMass FY Y v a ≤ 4 / 21 := by
...
theorem root_colour_single_charge_le
    ...
    ((flipSet FX.graph X u b).card : ℝ) * scaledMoveMass FX X u b -
      (((flipSet FX.graph X u b).card : ℝ) + 1) * scaledMoveMass FY Y v a ≤ 8 / 21 := by
  ...
  · have hb := (root_colour_single_allowed_iff h hn hv).mpr ⟨hl, ha⟩
    ...
    simpa only [Nat.cast_add, Nat.cast_one] using
      VigodaArithmetic.root_colour_one_neighbour_bound (flipSet FX.graph X u b).card
...
/-- Distinct affected off-root outputs in the first root-colour group. -/
def rootColourOffRows (FX FY : HardListInstance V C) (X Y : V → C) (v : V) :
    Finset (V → C) :=
  ((rootNeighbours FY Y v (X v)).image
    (fun u => flipConfiguration X (flipSet FX.graph X u (Y v)) (X u) (Y v))).filter
      (fun U => U v = X v)
...
lemma rootColourPartner_of_card_ne_one
    (FX FY : HardListInstance V C) (X Y : V → C) (v : V)
    (hn : (rootNeighbours FY Y v (X v)).card ≠ 1) :
    rootColourPartner FX FY X Y v = X := by
  apply dif_neg
  intro hp
  apply hn
  have hc := congrArg Finset.card hp.choose_spec.1
  exact hc.trans (Finset.card_singleton _)

omit [Nonempty V] [Nonempty C] in
lemma rootColourOffRows_single
    {FX FY : HardListInstance V C} {X Y : V → C} {v u : V} {a b : C}
    (h : RootLocalPair FX FY X Y v a b)
    (hn : rootNeighbours FY Y v a = {u}) (hv : v ∉ flipSet FX.graph X u b) :
    rootColourOffRows FX FY X Y v =
      {flipConfiguration X (flipSet FX.graph X u b) (X u) b} := by
  have hp : flipConfiguration X (flipSet FX.graph X u b) (X u) b v = X v :=
    flipConfiguration_of_not_mem hv
  simp only [rootColourOffRows, h.X_root, hn, h.Y_root, Finset.image_singleton,
    Finset.filter_singleton, show flipConfiguration X (flipSet FX.graph X u b) (X u) b v = a
      from hp.trans h.X_root, if_true]

omit [Nonempty V] [Nonempty C] in
lemma rootColourOffRows_single_shared
    {FX FY : HardListInstance V C} {X Y : V → C} {v u : V} {a b : C}
    (h : RootLocalPair FX FY X Y v a b)
    (hn : rootNeighbours FY Y v a = {u}) (hv : v ∈ flipSet FX.graph X u b) :
  ...
  have hXu : X u = a := (h.agree_off_root u (rootNeighbour_ne_root hu)).trans
    (mem_rootNeighbours.mp hu).2
  have hp : flipConfiguration X (flipSet FX.graph X u b) (X u) b v ≠ a := by
    rw [← h.X_root]
    exact (flipConfiguration_ne_iff (by simpa only [hXu] using h.colours_ne.symm) v).mpr hv
  simp only [rootColourOffRows, h.X_root, hn, h.Y_root, Finset.image_singleton,
    Finset.filter_singleton, if_neg hp]

omit [Fintype C] [Nonempty V] [Nonempty C] in
lemma rootColourPartner_single_shared
    {FX FY : HardListInstance V C} {X Y : V → C} {v u : V} {a b : C}
    (h : RootLocalPair FX FY X Y v a b)
    (hn : rootNeighbours FY Y v a = {u}) (hv : v ∈ flipSet FX.graph X u b) :
    rootColourPartner FX FY X Y v = X := by
  apply dif_neg
  rintro ⟨w, hw, hnot⟩
  have hw' : rootNeighbours FY Y v a = {w} := by simpa only [h.X_root] using hw
  have huw : u = w := Finset.singleton_inj.mp (hn.symm.trans hw')
  apply hnot
  simpa only [← huw, h.Y_root] using hv

lemma rootColourCharge_single_le
    {FX FY : HardListInstance V C} {X Y : V → C} {v u : V} {a b : C}
    (h : RootLocalPair FX FY X Y v a b)
    (hn : rootNeighbours FY Y v a = {u}) (hv : v ∉ flipSet FX.graph X u b)
    (hl : a ∈ FY.list v) : rootColourCharge FX FY X Y v ≤ 8 / 21 := by
  have hu : u ∈ rootNeighbours FY Y v a := by rw [hn]; simp
  have hXu : X u = a := (h.agree_off_root u (rootNeighbour_ne_root hu)).trans
    (mem_rootNeighbours.mp hu).2
...
lemma rootColourCharge_single_shared_le
    ...
    (h : RootLocalPair FX FY X Y v a b)
    (hn : rootNeighbours FY Y v a = {u}) (hv : v ∈ flipSet FX.graph X u b) :
    rootColourCharge FX FY X Y v ≤ 4 / 21 := by
...
theorem rootColourCharge_le
    ...
    rootColourCharge FX FY X Y v ≤
      (11 / 6 : ℝ) * (rootNeighbours FY Y v a).card - (if a ∈ FY.list v then 1 else 0) := by
