import ZeroFreeness.Coupling.Vigoda.HardFlipBoundary

/-! The exact mass of one feasible component move in the concrete hard kernel. -/
...
lemma flipConfiguration_swap_colours {X : V → C} {S : Finset V} {a b : C}
    (hab : a ≠ b) (hcol : ∀ w ∈ S, X w = a ∨ X w = b) :
    flipConfiguration X S a b = flipConfiguration X S b a := by
  funext w
  by_cases hw : w ∈ S
  · rw [flipConfiguration_of_mem hw, flipConfiguration_of_mem hw]
    rcases hcol w hw with ha | hb
    · simp [ha, hab]
    · simp [hb, hab.symm]
  · simp [flipConfiguration_of_not_mem hw]

/-- Proposing the new colour at any vertex in a component describes the
same component move, regardless of which colour it initially carries. -/
lemma component_move_from_member (G : SimpleGraph V) (X : V → C) (u : V) (c : C)
    (hc : c ≠ X u) (v : V) (hv : v ∈ flipSet G X u c) :
    let Y := flipConfiguration X (flipSet G X u c) (X u) c
    flipSet G X v (Y v) = flipSet G X u c ∧
      flipConfiguration X (flipSet G X v (Y v)) (X v) (Y v) = Y := by
  dsimp only
  let Y := flipConfiguration X (flipSet G X u c) (X u) c
  change flipSet G X v (Y v) = flipSet G X u c ∧
    flipConfiguration X (flipSet G X v (Y v)) (X v) (Y v) = Y
  have hne : X v ≠ Y v := ((flipConfiguration_ne_iff hc v).mpr hv).symm
  have hset : flipSet G X v (Y v) = flipSet G X u c :=
    (flipSet_eq_of_changes_root hne (show Y v = Y v from rfl)).symm
  refine ⟨hset, ?_⟩
  rw [hset]
  rcases proposal_pair_of_changes_root hne (show Y v = Y v from rfl) with
    ⟨hu, hy⟩ | ⟨hu, hy⟩
  · rw [← hu, ← hy]
  · rw [← hu, ← hy]
    exact (flipConfiguration_swap_colours hc.symm
      (fun w hw => colour_eq_of_mem_flipSet hw)).symm
...
lemma proposal_mass_of_component_member (F : HardListInstance V C) (X : V → C)
    ...
    (proposalDistribution F X v (Y v)).w Y =
      vigodaMass (flipSet F.graph X u c).card / (flipSet F.graph X u c).card := by
  let Y := flipConfiguration X (flipSet F.graph X u c) (X u) c
