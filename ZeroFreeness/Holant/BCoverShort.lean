import ZeroFreeness.Holant.Corollaries
...
`ZeroFreeness.Holant.bcover_uniform_polytube` states `∀ a c, ∃ ε`; here the width is
chosen before the upper endpoint `c = λ₊`.

Everything is derived from the library's proved Holant theorem
(`ZeroFreeness.Holant.holant_uniform_tubes`) through the library's proved reciprocal
reduction `ZeroFreeness.Holant.coverPartition_ne_zero_polytube`.
-/
