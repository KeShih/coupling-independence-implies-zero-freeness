import ZeroFreeness.Coupling.Girth.Tree.RatioMixing
import ZeroFreeness.Potts.Regions.Girth.Transfer.Family
import ZeroFreeness.Coupling.CLMM.SphereEstimate
import ZeroFreeness.Coupling.Girth.Tree.InfluenceIdentity
  ...
  have htree : ∀ (x : ℝ) (hx : x ∈ Ioo (0 : ℝ) 1),
    ...
    · intro d b t hd ht k a z
      have he : A * ρ ^ (k + 1) = totalInfluenceConstant Δ q * ρ ^ k := by
        ...
        field_simp
      rw [he]
      exact total_influence_decay (clmmInfluenceIdentity C) x hx.1 hx.2 hq d b t hd ht k a z
