import CI2ZF.Appendix.Edge.Slots.ConditionalLimit
import CI2ZF.Appendix.Edge.Slots.RootSemantics
import CI2ZF.Appendix.Edge.Finite.CI
import CI2ZF.RootCoupling
...
open PottsCI PottsCI.FinDist CI2ZF.Potts EndpointGeometry FiniteSystem
...
theorem root_children_ci (G : SimpleGraph V) (τ : PartialColouring G.edgeSet C)
  ...
  · have hx : x = PinningData.hardParameter := Subtype.ext hz
    ...
    apply (rootChildData τ G.lineGraph r a).hard_W_le_of_positive_parameter_bound
      ...
      (rootChildData_degreeBound τ G.lineGraph r a (lineGraph_degree_le G hdegree))
      (rootChildData_degreeBound τ G.lineGraph r b (lineGraph_degree_le G hdegree))
      (lineGraph_colours_slack hΔ hq) CI2ZF.hardApproach CI2ZF.tendsto_hardApproach
      (Filter.Eventually.of_forall CI2ZF.hardApproach_pos)
      (Filter.Eventually.of_forall CI2ZF.hardApproach_le_one)
