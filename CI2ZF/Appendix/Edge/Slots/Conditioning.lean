import CI2ZF.Appendix.Edge.Slots.SimpleGraphBounds
import CI2ZF.Appendix.Edge.Finite.ConditionalCosts
import PottsCI.EndpointContinuity
...
lemma eventMass_mapLaw (μ : FinDist S) (f : S → T) (P : T → Prop) :
    eventMass (CI2ZF.mapLaw μ f) P = eventMass μ (fun x => P (f x)) := by
  have h := CI2ZF.expectReal_mapLaw μ f (fun y => if P y then (1 : ℝ) else 0)
  simpa only [CI2ZF.expectReal, mul_ite, mul_one, mul_zero, eventMass] using h

theorem conditional_mapLaw (μ : FinDist S) (f : S → T) (P : T → Prop) :
    CI2ZF.mapLaw (conditional μ (fun x => P (f x))) f = conditional (CI2ZF.mapLaw μ f) P := by
  by_cases hp : 0 < eventMass μ (fun x => P (f x))
  · have hpm : 0 < eventMass (CI2ZF.mapLaw μ f) P := by rwa [eventMass_mapLaw]
    apply FinDist.ext
    funext y
...
lemma mapLaw_tendsto {μ : ℕ → FinDist S} {μ₀ : FinDist S}
    ...
    Tendsto (fun n => (CI2ZF.mapLaw (μ n) f).w y) atTop (𝓝 ((CI2ZF.mapLaw μ₀ f).w y)) := by
...
def rootExcludedColourLaw (μ : FinDist (E → C)) (e : E) (c : C) :
    FinDist ({f : E // f ≠ e} → C) :=
  CI2ZF.mapLaw (conditional μ (fun φ => φ e = c)) (restrictConfig e)
...
lemma rootExcludedColourLaw_eq (μ : FinDist (E → SlotState C R)) (e : E) (c : C) :
    rootExcludedColourLaw (CI2ZF.mapLaw μ colourProjection) e c =
      CI2ZF.mapLaw (conditional μ (fun σ => (σ e).1 = c)) (remainingColourProjection e) := by
  rw [rootExcludedColourLaw, ← conditional_mapLaw, CI2ZF.mapLaw_comp]
  rfl
...
lemma W_remaining_restore_le (e : E) (a b : SlotState C R)
    ...
    W (fun σ τ => ham (remainingColourProjection e σ) (remainingColourProjection e τ))
      (CI2ZF.mapLaw μ (joinConfig e a)) (CI2ZF.mapLaw ν (joinConfig e b)) ≤ W ham μ ν := by
  obtain ⟨π, hπ⟩ := exists_optimal_coupling μ ν ham ham_nonneg
  apply (W_le_cost (fun _ _ => ham_nonneg _ _)
    (CI2ZF.mapCoupling π (joinConfig e a) (joinConfig e b))).trans
  rw [CI2ZF.cost_mapCoupling, ← hπ]
  apply CI2ZF.cost_le_cost π
  intro σ τ
...
lemma colour_coordinate_data (I : FiniteSystem V E (SlotState C R) (C × R))
    ...
    I.Compatible B e a ∧
      conditional (conditional (I.validLaw B hB) (fun σ => (σ e).1 = c)) (fun σ => σ e = a) =
        CI2ZF.mapLaw (I.pinnedLaw B hB e a) (joinConfig e a) := by
...
theorem rootExcludedColourLaw_W_le (I : FiniteSystem V E (SlotState C R) (C × R))
    ...
    W ham (rootExcludedColourLaw (CI2ZF.mapLaw (I.validLaw B hB) colourProjection) e c)
      (rootExcludedColourLaw (CI2ZF.mapLaw (I.validLaw B hB) colourProjection) e d) ≤ K := by
  rw [rootExcludedColourLaw_eq, rootExcludedColourLaw_eq]
  apply (CI2ZF.W_mapLaw_le _ _ ham ham_nonneg).trans
