import CI2ZF.Appendix.Girth.Transfer.WeightedInfluence
import CI2ZF.Appendix.Common.CLMM.AmbientDegree
import CI2ZF.CouplingIndependence
...
theorem uniformWeightedSource_one (Δ g : ℕ) (χ : ℝ) {M : ℝ} (hM : 0 ≤ M) :
  ...
  rw [CI2ZF.gibbs_one_eq (optionChildData I a) (optionChildData I b) ha hb]
...
theorem coupling_closed_of_positive {O : Type u} [Fintype O]
  ...
  · subst x
    ...
    let μ (D : PinningData O C) (n : ℕ) := D.gibbs (CI2ZF.hardApproach n)
      (CI2ZF.hardApproach n).property
      (D.partition_pos_of_parameter_pos (CI2ZF.hardApproach_pos n))
    ...
    exact Filter.Eventually.of_forall fun n => hpos (CI2ZF.hardApproach n)
