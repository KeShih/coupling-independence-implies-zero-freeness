import CI2ZF.Coupling.Girth.Covariance.Insertion.LocalSource
...
theorem conditional_source (fU : U → C → ℝ) (fS : Ω → ℝ) (fO : O → ℝ) (ξ : Ω) :
  ...
  rw [expectReal_sum]
  simp_rw [CI2ZF.expectReal_product_coordinate]
  rfl
...
theorem conditional_full_covariance (s : ℝ) (c : C)
    ...
    covariance (M.conditionalLaw ξ) (fun τ => additiveSource fU fS fO (ξ,τ))
      ...
      covariance (CI2ZF.productLaw (M.cavity ξ)) (fun α => ∑ u, fU u (α u))
        (M.residual s c) := by
