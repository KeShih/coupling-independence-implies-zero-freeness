...
import CI2ZF.Coupling.Foundations.PartialCombining
...
local instance cvGlobalRegularFunctionDecEq : DecidableEq (V → C) := Classical.decEq _
...
/-- A selected regular-colour plan, using any legal actual selectors. -/
def selectedRegularPartial [Nonempty V] [Nonempty C]
  ...
  if (rootNeighbours FX X v c).Nonempty then
    canonicalRegularPartial FX FY X Y v c (choice.left c) (choice.right c)
  else singletonRegularPartial FX FY X Y v c

lemma selectedRegularPartial_ge_common [Nonempty V] [Nonempty C]
    {FX FY : HardListInstance V C} {X Y : V → C} {v : V} {a b : C}
    (h : RootLocalPair FX FY X Y v a b) (choice : GlobalChoice h) (c : C) (U Z : V → C) :
    (hardCommonOffRootPartial FX FY X Y v).w U Z ≤
      (selectedRegularPartial h choice c).w U Z := by
  unfold selectedRegularPartial
  split_ifs
  · exact canonicalRegularPartial_ge_common _ _ _ _ _ _ _ _ _ _
...
lemma selectedRegularPartial_eq_common_off_rows [Nonempty V] [Nonempty C]
  ...
  · have hNY : (rootNeighbours FY Y v c).Nonempty := (h.rootNeighbours_eq hca hcb) ▸ hN
    exact canonicalRegularPartial_eq_common_off_rows h hca hcb _ _
      (choice.left_mem c hN) (choice.right_mem c hNY) U Z hU
...
lemma selectedRegularPartial_eq_common_off_cols [Nonempty V] [Nonempty C]
  ...
  split_ifs with hN
  · have hNY : (rootNeighbours FY Y v c).Nonempty := (h.rootNeighbours_eq hca hcb) ▸ hN
    exact canonicalRegularPartial_eq_common_off_cols h hca hcb _ _
      (choice.left_mem c hN) (choice.right_mem c hNY) U Z hZ
...
def globalRegularPartial [Nonempty V] [Nonempty C]
  ...
  PartialCoupling.combineDisjoint (hardCommonOffRootPartial FX FY X Y v)
    (fun c : RegularColour a b => selectedRegularPartial h choice c.val)
    (fun c : RegularColour a b => regularMoveRows FX X v b c.val)
    (fun c : RegularColour a b => regularMoveRows FY Y v a c.val)
    (fun c => selectedRegularPartial_ge_common h choice c.val)
    (fun c => selectedRegularPartial_eq_common_off_rows h choice c.property.1 c.property.2)
    (fun c => selectedRegularPartial_eq_common_off_cols h choice c.property.1 c.property.2)
...
lemma globalRegularPartial_w [Nonempty V] [Nonempty C]
    ...
    (globalRegularPartial h choice).w U Z = (hardCommonOffRootPartial FX FY X Y v).w U Z +
      ∑ c : RegularColour a b,
        ((selectedRegularPartial h choice c.val).w U Z -
...
lemma globalRegularPartial_eq_common_off_rows [Nonempty V] [Nonempty C]
  ...
  have hz (c : RegularColour a b) :
    ...
    rw [selectedRegularPartial_eq_common_off_rows h choice c.property.1 c.property.2 U Z (hU c), sub_self]
...
lemma globalRegularPartial_eq_common_off_cols [Nonempty V] [Nonempty C]
  ...
  have hz (c : RegularColour a b) :
      (selectedRegularPartial h choice c.val).w U Z -
        (hardCommonOffRootPartial FX FY X Y v).w U Z = 0 := by
    rw [selectedRegularPartial_eq_common_off_cols h choice c.property.1 c.property.2 U Z (hZ c), sub_self]
  simp only [hz, Finset.sum_const_zero, add_zero]

lemma globalRegularPartial_eq_selected_on_rows [Nonempty V] [Nonempty C]
    {FX FY : HardListInstance V C} {X Y : V → C} {v : V} {a b : C}
    (h : RootLocalPair FX FY X Y v a b) (choice : GlobalChoice h) (c : RegularColour a b) (U Z : V → C)
    (hU : U ∈ regularMoveRows FX X v b c.val) :
    (globalRegularPartial h choice).w U Z = (selectedRegularPartial h choice c.val).w U Z := by
  ...
  · intro d _ hdc
    ...
    rw [selectedRegularPartial_eq_common_off_rows h choice d.property.1 d.property.2 U Z hUd, sub_self]
  · simp
...
lemma globalRegularPartial_eq_selected_on_cols [Nonempty V] [Nonempty C]
  ...
  · intro d _ hdc
    ...
    rw [selectedRegularPartial_eq_common_off_cols h choice d.property.1 d.property.2 U Z hZd, sub_self]
  · simp
...
def globalRegularCoupling [Nonempty V] [Nonempty C]
  ...
  (globalRegularPartial h choice).complete
