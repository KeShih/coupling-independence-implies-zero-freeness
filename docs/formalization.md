# The formalization in detail

The details behind the [repository README](../README.md): the Lean names of the
main results, what each status in the coverage table means, what remains
unformalized, the cited results proved in Lean, and how to build, verify and
update the site.
...
- **Every numbered result.** Every numbered theorem, lemma, proposition and
  ...
  [docs/coverage.json](coverage.json) lists all 107 numbered statements,
...
[docs/appendix/STATUS.md](appendix/STATUS.md) gives the exact
...
[docs/coverage.json](coverage.json) has one entry for each of the 107
...
of harmonic numbers. [docs/external-inputs.md](external-inputs.md)
...
only the companion. [docs/verification.json](verification.json) records
...
[documentation index](README.md) or the [proof overview](overview.md).
